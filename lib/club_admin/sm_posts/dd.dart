...
class PlayersTablePageState extends State<PlayersTablePage> {
  ...
  Future<List<PlayersTable>> getFilteredPlayers(String clubId) async {
    // Fetch visibility data for all teams
    CollectionReference visibilityRef = FirebaseFirestore.instance.collection('clubs').doc(clubId).collection('TeamClassVisibility');
    QuerySnapshot visibilitySnapshot = await visibilityRef.get();

    Map<String, bool> teamVisibility = {};

    for (var doc in visibilitySnapshot.docs) {
      teamVisibility[doc.id] = doc['isVisible'] as bool;
      print('Team visibility: $teamVisibility');
    }

    // Fetch all players from PlayersTable
    QuerySnapshot playersSnapshot = await FirebaseFirestore.instance.collection('clubs').doc(clubId).collection('PllayersTable').get();
    print(playersSnapshot.docs.map((doc) => doc.data()).toList()); // Print data for debugging

    List<PlayersTable> filteredPlayers = [];

    for (var playerDoc in playersSnapshot.docs) {
      PlayersTable player = PlayersTable.fromMap(playerDoc.data() as Map<String, dynamic>);

      String teamCollection = '';
      if (await _isPlayerInTeam(player.playerName!, 'FirstTeamClassPlayers', clubId)) {
        teamCollection = 'FirstTeamClass';
      } else if (await _isPlayerInTeam(player.playerName!, 'SecondTeamClassPlayers', clubId)) {
        teamCollection = 'SecondTeamClass';
      } else if (await _isPlayerInTeam(player.playerName!, 'ThirdTeamClassPlayers', clubId)) {
        teamCollection = 'ThirdTeamClass';
      } else if (await _isPlayerInTeam(player.playerName!, 'FourthTeamClassPlayers', clubId)) {
        teamCollection = 'FourthTeamClass';
      } else if (await _isPlayerInTeam(player.playerName!, 'FifthTeamClassPlayers', clubId)) {
        teamCollection = 'FifthTeamClass';
      } else if (await _isPlayerInTeam(player.playerName!, 'SixthTeamClassPlayers', clubId)) {
        teamCollection = 'SixthTeamClass';
      }
      // Check if the team class is visible
      if (teamVisibility[teamCollection] == true) {
        filteredPlayers.add(player);
      }
      print('Checking player: ${player.playerName} in collection: $teamCollection');
    }

    return filteredPlayers;
  }

  Future<bool> _isPlayerInTeam(String playerName, String teamCollection, String clubId) async {
    QuerySnapshot snapshot =
        await FirebaseFirestore.instance.collection('clubs').doc(clubId).collection(teamCollection).where('name', isEqualTo: playerName).get();
  ...
  void _fetchAndSetPlayers() async {
    List<PlayersTable> filteredPlayers = await getFilteredPlayers(widget.clubId);

    setState(() {
      playersTableList = filteredPlayers; // Update the players list with filtered players
      playersTableDataSource.playersTableList = filteredPlayers; // Update the data source
      playersTableDataSource._buildDataRow(); // Rebuild the data grid rows
      playersTableDataSource.updateDataGridSource(); // Notify listeners
    });
  }
...
class PlayersTable {
  ...
  // This is the fromMap method that converts a Map to a PlayersTable object
  factory PlayersTable.fromMap(Map<String, dynamic> map) {
    return PlayersTable(
      id: map['id'] as String?,
      image: map['image'] as String?,
      playerName: map['player_name'] as String?,
      matchesPlayed: map['matches_played'] as int?,
      matchesStarted: map['matches_started'] as int?,
      matchesBenched: map['matches_benched'] as int?,
      goalsScored: map['goals_scored'] as int?,
      assists: map['assists'] as int?,
      playerPosition: map['player_position'] as String?,
      yellowCard: map['yellow_card'] as int?,
      redCard: map['red_card'] as int?,
