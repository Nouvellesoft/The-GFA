...
import '../../../main.dart';
import 'display_yellow_card_history_page.dart';
import 'record_yellow_card_page.dart';
...
class TabviewYellowCardPage extends StatefulWidget implements NavigationStates {
  final String clubId;
  const TabviewYellowCardPage({super.key, required this.clubId});

  @override
  State<TabviewYellowCardPage> createState() => TabviewYellowCardPageState();
}

class TabviewYellowCardPageState extends State<TabviewYellowCardPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: backgroundColor,
        title: Text(mcTitle, style: GoogleFonts.cairo(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.white)),
        leading: IconButton(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          MyRecordYellowCardPage(clubId: widget.clubId),
          MyDisplayYellowCardHistoryPage(clubId: widget.clubId),
        ],
      ),
    );
  }
