...
class TabviewCaptainsPage extends StatefulWidget implements NavigationStates {
  final String clubId;
  const TabviewCaptainsPage({super.key, required this.clubId});
...
class TabviewCaptainsPageState extends State<TabviewCaptainsPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: backgroundColor,
        title: Text(mcTitle, style: GoogleFonts.jura(fontSize: 23, fontWeight: FontWeight.w800, color: Colors.white)),
        leading: IconButton(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          MyModifyAddClubCaptainsPage(clubId: widget.clubId),
          MyModifyRemoveClubCaptainsPage(clubId: widget.clubId),
        ],
      ),
    );
  }
